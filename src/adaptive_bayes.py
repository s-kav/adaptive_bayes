...
def _update_batch_numba(X, y, w, base_lr, eps):
    ...
    for i in prange(n):
        ...
        # ИСПРАВЛЕНИЕ: обновление с исходными значениями X
        for j in range(d):
            x = X[i, j]  # Исходное значение, не лог-преобразованное
            if abs(x) > eps:
                w[j] += lr * err * x  # Градиент по исходному x
...
class AdaptiveBayes:
    ...
    def _ensure_weights(self, d):
        if self.w is None:
            if self._is_gpu:
                self.w = self._xp.random.normal(0, 0.01, d).astype(self._xp.float64)
            else:
                self.w = np.random.normal(0, 0.01, d).astype(np.float64)
        else:
            if (self._is_gpu and self.w.shape[0] != d) or ((not self._is_gpu) and self.w.shape[0] != d):
                if self._is_gpu:
                    self.w = self._xp.random.normal(0, 0.01, d).astype(self._xp.float64)
                else:
                    self.w = np.random.normal(0, 0.01, d).astype(np.float64)
    ...
    def _update_gpu(self, X, y):
        ...
        # ИСПРАВЛЕНИЕ: обновление с исходными X (не лог-преобразованными)
        coeff = (lr * err)[:, None]
        delta = (coeff * self._xp.where(X_mask, X, 0.0)).sum(axis=0)  # Используем X, не X_log!
        self.w += delta
    ...
    def fit(self, X, y, epochs=1, batch_size=65536, shuffle=True):
        ...
        if not self._is_gpu and isinstance(y, np.ndarray) is False:
            y = np.asarray(y, dtype=np.float64)
        if self._is_gpu and isinstance(y, cp.ndarray) is False:
